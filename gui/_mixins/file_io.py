...
- _save_window_screenshot: ウィンドウ全体 PNG 保存
...
class FileIOMixin:
    ...
    def _copy_sgf(self: "MainWindowProto"):
        ...
        try:
            ...
            self.show_toast("SGF をクリップボードにコピーしました")
        except Exception:
            # エラー通知は今回のスコープ外(ステータスバー廃止に伴い無音化)。
            pass
        finally:
            # 一時ファイル削除
    ...
    def _save_window_screenshot(self: "MainWindowProto"):
        """ウィンドウ全体（タイトルバー込み）を PNG でファイル保存する。
        保存先のデフォルトはデスクトップ。
        """
        desktop = QStandardPaths.writableLocation(QStandardPaths.StandardLocation.DesktopLocation)
        path, _ = QFileDialog.getSaveFileName(
